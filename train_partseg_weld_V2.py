...
from dataset import PartNormalDataset, WeldDataset
import hydra
import omegaconf
...
@hydra.main(config_path='config', config_name='partseg_v2')
def main(args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, label, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            # 生成 edge_index （图结构）
            # edge_index = knn_graph(points[:, :, 0:3].view(-1, 3), k=6).cuda()  # 新增部分

            optimizer.zero_grad()

            cat_points = torch.cat([points, to_categorical(label, num_category).repeat(1, points.shape[1], 1)], -1)

            seg_pred = classifier(cat_points)
            # seg_pred = classifier(torch.cat([points, to_categorical(label, num_category).repeat(1, points.shape[1], 1)], -1), edge_index)[1]   ############ for pointTV2-other model users
