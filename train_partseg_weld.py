...
@hydra.main(config_path='config', config_name='partseg-o')
def main(args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, label, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            seg_pred = classifier(torch.cat([points, to_categorical(label, num_category).repeat(1, points.shape[1], 1)], -1)) ############for pointTV1-partseg
            # seg_pred = classifier(torch.cat([points, to_categorical(label, num_category).repeat(1, points.shape[1], 1)], -1))[1]   ############ for pointTV2-other model users
