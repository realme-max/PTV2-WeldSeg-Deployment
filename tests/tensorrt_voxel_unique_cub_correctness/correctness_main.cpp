...
constexpr uint64_t kValuesCapacityBytes{static_cast<uint64_t>(kMaxN) * sizeof(int64_t)};
...
template <typename T> struct TrtDeleter { void operator()(T* object) const noexcept { delete object; } };
...
Reference cpuUniqueReference(std::vector<int64_t> const& keys)
{
    ...
    {
        auto const iterator = std::lower_bound(result.values.begin(), result.values.end(), key);
        result.inverse.push_back(static_cast<int64_t>(std::distance(result.values.begin(), iterator)));
    }
    return result;
}

struct TestCase { std::string name; std::vector<int64_t> keys; };
...
std::vector<TestCase> makeTestCases()
{
    ...
    for (int32_t n : {4, 8, 32, 2048})
    {
        ...
        for (int32_t repetition = 0; repetition < 3; ++repetition)
        {
            ...
            for (int64_t& key : keys) key = static_cast<int64_t>(random() % span) - radius;
            cases.push_back({"random_n" + std::to_string(n) + "_r" + std::to_string(repetition), std::move(keys)});
        }
    }
    cases.push_back({"n1", {std::numeric_limits<int64_t>::min()}});
    ...
    cases.push_back({"mixed_signs", {-9, 4, 0, -1, 4, 8, -9, 1, 0, -3}});
    ...
    cases.push_back({"mixed_minmax", {
        std::numeric_limits<int64_t>::max(), -17, std::numeric_limits<int64_t>::min(),
        17, 0, -17, std::numeric_limits<int64_t>::max(), std::numeric_limits<int64_t>::min()}});
    return cases;
}
...
{
public:
    ValuesOutputAllocator(void* memory, uint64_t capacity) : mMemory(memory), mCapacity(capacity) {}
    void reset() noexcept { mRequestedBytes = 0; mShapeNotified = false; mShape = {}; }
    void* reallocateOutputAsync(char const*, void* currentMemory, uint64_t size, uint64_t, cudaStream_t) override
    {
        mRequestedBytes = size;
        return (currentMemory == nullptr || currentMemory == mMemory) && size <= mCapacity ? mMemory : nullptr;
    }
    void notifyShape(char const*, nvinfer1::Dims const& dims) noexcept override
    {
        mShape = dims;
        mShapeNotified = true;
    }
...
template <typename T> void writeVector(std::ostream& stream, std::vector<T> const& values)
{
    stream << '[';
    for (size_t i = 0; i < values.size(); ++i) { if (i != 0) stream << ','; stream << values[i]; }
    stream << ']';
}
...
void writeComparison(std::string const& path, std::vector<CaseResult> const& results)
{
    ...
    output << "{\n  \"algorithm\": \"cub_radix_sort_run_length_inverse\",\n"
           << "  \"sorted\": true,\n  \"case_count\": " << results.size() << ",\n"
           << "  \"all_passed\": "
           << (std::all_of(results.begin(), results.end(), [](auto const& value) { return value.passed; }) ? "true" : "false")
           << ",\n  \"cases\": [\n";
...
void writeWorkspace(std::string const& path, WorkspaceLayout const& layout)
{
    std::ofstream output(path);
    output << "{\n  \"alignment_bytes\": 256,\n  \"profile_max_n\": 2048,\n"
           << "  \"sorted_keys\": {\"offset\": " << layout.sortedKeysOffset << ", \"bytes\": 16384},\n"
           << "  \"original_indices\": {\"offset\": " << layout.originalIndicesOffset << ", \"bytes\": 8192},\n"
           << "  \"sorted_original_indices\": {\"offset\": " << layout.sortedOriginalIndicesOffset << ", \"bytes\": 8192},\n"
           << "  \"boundary_flags\": {\"offset\": " << layout.boundaryFlagsOffset << ", \"bytes\": 8192},\n"
           << "  \"unique_ids\": {\"offset\": " << layout.uniqueIdsOffset << ", \"bytes\": 8192},\n"
           << "  \"cub_temporary\": {\"offset\": " << layout.cubTemporaryOffset
           << ", \"radix_sort_bytes\": " << layout.radixSortTemporaryBytes
           << ", \"inclusive_scan_bytes\": " << layout.scanTemporaryBytes
           << ", \"shared_capacity_bytes\": " << layout.cubTemporaryBytes << "},\n"
           << "  \"total_bytes\": " << layout.totalBytes << ",\n"
           << "  \"enqueue_dynamic_allocations\": false,\n"
           << "  \"enqueue_host_round_trip\": false,\n"
           << "  \"enqueue_synchronization\": false\n}\n";
}
} // namespace
...
int main(int argc, char** argv)
{
    if (argc != 6)
    {
        std::cerr << "Usage: voxel_unique_cub_correctness <plugin.dll> <custom.onnx> <engine.plan> <comparison.json> <workspace.json>\n";
        return 64;
    }
    try
    {
        HMODULE library = LoadLibraryA(argv[1]);
        if (library == nullptr) throw std::runtime_error("LoadLibrary failed for experimental plugin DLL");
        using InitFunction = bool (*)() noexcept;
        using WorkspaceFunction = bool (*)(int32_t, WorkspaceLayout*) noexcept;
        auto init = reinterpret_cast<InitFunction>(GetProcAddress(library, "initVoxelUniqueCubPlugin"));
        auto workspaceQuery = reinterpret_cast<WorkspaceFunction>(GetProcAddress(library, "getVoxelUniqueCubWorkspaceLayout"));
        if (init == nullptr || workspaceQuery == nullptr || !init())
            throw std::runtime_error("Experimental plugin registration/export lookup failed");
        WorkspaceLayout layout{};
        if (!workspaceQuery(kMaxN, &layout) || layout.totalBytes == 0)
            throw std::runtime_error("Workspace layout query failed");
        writeWorkspace(argv[5], layout);

        Logger logger;
        initLibNvInferPlugins(&logger, "");
        TrtUniquePtr<nvinfer1::IBuilder> builder{nvinfer1::createInferBuilder(logger)};
        TrtUniquePtr<nvinfer1::INetworkDefinition> network{builder ? builder->createNetworkV2(0U) : nullptr};
        TrtUniquePtr<nvinfer1::IBuilderConfig> config{builder ? builder->createBuilderConfig() : nullptr};
        ...
        if (!parser || !parser->parseFromFile(argv[2], static_cast<int32_t>(nvinfer1::ILogger::Severity::kERROR)))
        {
            if (parser) for (int32_t i = 0; i < parser->getNbErrors(); ++i) std::cerr << "PARSER_ERROR=" << parser->getError(i)->desc() << '\n';
            throw std::runtime_error("Experimental correctness ONNX parse failed");
        }
        ...
        if (!serialized) throw std::runtime_error("Experimental correctness engine build failed");
        ...
        TrtUniquePtr<nvinfer1::ICudaEngine> engine{runtime ? runtime->deserializeCudaEngine(serialized->data(), serialized->size()) : nullptr};
        ...
        for (TestCase const& test : makeTestCases())
        {
            ...
            if (!context->setInputShape("voxel_key", inputDims)) throw std::runtime_error("setInputShape failed for " + test.name);
            allocator.reset();
            CUDA_CHECK(cudaMemcpyAsync(deviceKeys, test.keys.data(), test.keys.size() * sizeof(int64_t), cudaMemcpyHostToDevice, stream));
            if (!context->enqueueV3(stream)) throw std::runtime_error("enqueueV3 failed for " + test.name);
            ...
            if (pluginCount < 1 || pluginCount > static_cast<int32_t>(test.keys.size())) throw std::runtime_error("Invalid plugin count for " + test.name);
            values.resize(static_cast<size_t>(pluginCount));
            int64_t runtimeM = allocator.shapeNotified() && allocator.shape().nbDims == 1 ? allocator.shape().d[0] : -1;
            CaseResult item;
            item.test = test;
            ...
            std::cout << "CASE name=" << test.name << " n=" << test.keys.size() << " m=" << pluginCount << " passed=" << std::boolalpha << item.passed << '\n';
            results.push_back(std::move(item));
        }
        writeComparison(argv[4], results);
        bool const allPassed = std::all_of(results.begin(), results.end(), [](CaseResult const& item) { return item.passed; });
        std::cout << "CASE_COUNT=" << results.size() << '\n';
        std::cout << "WORKSPACE_BYTES_N2048=" << layout.totalBytes << '\n';
        ...
        if (!allPassed) { std::cerr << "VOXEL_UNIQUE_CUB_CORRECTNESS_FAILED\n"; return 2; }
        std::cout << "VOXEL_UNIQUE_CUB_CORRECTNESS_PASSED\n";
        return 0;
    }
    catch (std::exception const& error)
    {
        std::cerr << "ERROR=" << error.what() << '\n';
        std::cerr << "VOXEL_UNIQUE_CUB_CORRECTNESS_FAILED\n";
        return 1;
    }
}
