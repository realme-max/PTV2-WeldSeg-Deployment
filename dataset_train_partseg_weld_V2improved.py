...
from dataset import WeldDataset, S3DISDataset, PartNormalDataset
...
def main(args):
    ...
    root = hydra.utils.to_absolute_path('D:/xlxlqqq/document/pointnet/Pointnet_Pointnet2_pytorch-master/data/shapenetcore_partanno_segmentation_benchmark_v0_normal/')
    # root = hydra.utils.to_absolute_path('D:/xlxlqqq/document/pointnet/Pointnet_Pointnet2_pytorch-master/data/weld/')

    # # 训练集（排除Area 5）
    # train_dataset = S3DISDataset(split='test', test_area=5, npoints=2048)
    ...
    # )
    # point = train_loader.dataset.room_labels
    # for i, (points, label, target), something in tqdm(enumerate(train_loader), total=len(train_loader), smoothing=0.9):
    #     labels = label
    #     points = point
    #     targets = target
    #     print(0)
    #
    # # 测试集（仅Area 5）
    # val_dataset = S3DISDataset(root = "Q:/xlxlqqq/documents/dataset/Stanford3dDataset_v1.2_lite", split='test', test_area=5, npoints=2048)
    ...
    try:
        # checkpoint = torch.load('./models/testParameters/best_model.pth')
    ...
    # summary(classifier, input_size=(cat_points, adj))
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, label, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            # cat_points = local_feature_fusion(cat_points.cuda(), k=8)

            # seg_pred = classifier(cat_points, adj)  # ############改进：新增 edge_index
            seg_pred = classifier(torch.cat([points, to_categorical(label, num_category).repeat(1, points.shape[1], 1)], -1), adj)   ############ for pointTV2-other model users
            # seg_pred = classifier(torch.cat([points, to_categorical(label, num_category).repeat(1, points.shape[1], 1)], -1))
            seg_pred = seg_pred[1].contiguous().view(-1, num_part)
        ...
        with torch.no_grad():
            ...
            for batch_id, (points, label, target) in tqdm(enumerate(testDataLoader), total=len(testDataLoader), smoothing=0.9):
                ...
                points, label, target = points.float().cuda(), label.long().cuda(), target.long().cuda()
                adj = compute_adjacency(points)
                tempInput = torch.cat([points, to_categorical(label, num_category).repeat(1, points.shape[1], 1)], -1) ##
                seg_pred = classifier(torch.cat([points, to_categorical(label, num_category).repeat(1, points.shape[1], 1)], -1), adj)[1] #############
                # seg_pred = classifier(torch.cat([points, to_categorical(label, num_category).repeat(1, points.shape[1], 1)], -1))[1]
