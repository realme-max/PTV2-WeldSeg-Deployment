...
def drawLFA(imagePath, points, features):
    features = features.cpu().numpy()[0]  # LFA之后的特征张量
    points_xyz = points.cpu().numpy()[0]  # [2048, 3]，原始点的位置坐标

    # 用 t-SNE 将高维特征降到3维，用于 RGB 显示
    features_3d = TSNE(n_components=3, learning_rate='auto', init='pca', perplexity=4).fit_transform(features)
    # features_3d = TSNE(n_components=2, perplexity=4).fit_transform(features)

    # 标准化到 [0, 255]
    features_3d_norm = (features_3d - features_3d.min(0)) / (features_3d.max(0) - features_3d.min(0))
    colors = (features_3d_norm * 255).astype(np.uint8)

    # 用 matplotlib 显示 3D 彩色点云
    from mpl_toolkits.mplot3d import Axes3D

    fig = plt.figure(figsize=(10, 8))
    ax = fig.add_subplot(111, projection='3d')
    ax.scatter(points_xyz[:, 0], points_xyz[:, 1], points_xyz[:, 2], c=colors / 255.0, s=1)
    plt.title("LFA后特征的t-SNE可视化")
    ax.grid(False)
    ax.set_xticks([])
    ax.set_yticks([])
    ax.set_zticks([])
    ax.set_axis_off()  # 👈 彻底关闭三维坐标轴（包括轴线和边框）
    # plt.show()

    feature_channel = features[:, 3]
    norm_feature = (feature_channel - feature_channel.min()) / (feature_channel.max() - feature_channel.min())

    # 映射成颜色（用 colormap）
    import matplotlib.cm as cm
    colors = cm.viridis(norm_feature)

    # 3D 可视化
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    ax.scatter(points_xyz[:, 0], points_xyz[:, 1], points_xyz[:, 2], c=colors, s=1)
    plt.title("某通道LFA特征可视化")
    ax.grid(False)
    ax.set_xticks([])
    ax.set_yticks([])
    ax.set_zticks([])
...
class PTV2Segmentation(nn.Module):
    ...
    # def __init__(self, cfg, in_dim=19):
        ...
        self.LFA = LocalFeatureAggregationBest(input_dim=4, output_dim=4, k=8)
    ...
    def forward(self, points, adj):
        ...
        drawLFA("./data/test_output/LFAFigure.png", points_xyz, points_features)
