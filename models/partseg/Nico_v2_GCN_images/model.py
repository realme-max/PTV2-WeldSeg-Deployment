...
def savaFeatures(imagePath, tensorArray):
    image = tensorArray.cpu().numpy().astype(np.uint8)
...
class PTV2Segmentation(nn.Module):
    ...
    def __init__(self, cfg, in_dim = 4):
        ...
        # self.linear_2 = nn.Linear(384, 384)
    ...
    def forward(self, points, adj):
        ...
        points_xyz, points_features = points[:, :, :3], points   # points_feature: 1*2048*4
        out = self.linear_1(points_features) #out: 1*2048*48

        out_xyz, out_features = self.ptb_0(points_xyz, out) # out_features: 1*2048*48
        # out_features = self.gcn_0(out_features, adj)  # 添加GCN操作
        skipped_0_xyz, skipped_0_features = torch.clone(out_xyz), torch.clone(out_features)
        savaFeatures("./data/test_output/0.png", out_features[0, :, :])

        out_xyz, out_features = self.tdb_1(out_xyz, out_features)
        out_xyz, out_features = self.ptb_1(out_xyz, out_features) # out_feautures: 1*403*96
        skipped_1_xyz, skipped_1_features = torch.clone(out_xyz), torch.clone(out_features)

        out_xyz, out_features = self.tdb_2(out_xyz, out_features)
        out_xyz, out_features = self.ptb_2(out_xyz, out_features) # out_feautures: 1*98*192
        skipped_2_xyz, skipped_2_features = torch.clone(out_xyz), torch.clone(out_features)

        out_xyz, out_features = self.tdb_3(out_xyz, out_features)
        out_xyz, out_features = self.ptb_3(out_xyz, out_features) # out_feautures: 1*13*384
        skipped_3_xyz, skipped_3_features = torch.clone(out_xyz), torch.clone(out_features)

        out_xyz, out_features = self.tdb_4(out_xyz, out_features)
        out_xyz, out_features = self.ptb_4(out_xyz, out_features) # out_feautures: 1*3*512
