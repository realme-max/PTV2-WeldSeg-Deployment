...
def GridCluster(points, grid_size):
    """
    重新实现 torch_cluster.grid.grid_cluster，保证输入参数和功能一致
    :param points: Tensor, 形状 [N, 3]，点云坐标
    :param grid_size: Tensor, 形状 [3]，网格尺寸
    :return: cluster_indices, Tensor, 形状 [N]，每个点的网格索引
    """
    # 计算每个点的网格坐标（归一化到网格中）
    grid_coords = (points / grid_size).floor().long()

    # 计算最大坐标值，确保索引唯一
    max_vals = grid_coords.max(dim=0).values + 1  # 避免哈希冲突

    # 3D 网格索引转换为 1D 索引
    cluster_indices = grid_coords[:, 0] + grid_coords[:, 1] * max_vals[0] + grid_coords[:, 2] * max_vals[0] * max_vals[1]
...
# 使用其它方式对expand进行重写，避免pytorch和onnx对于expand的处理方式不一样
# 随后即可正常导出IR
def index_points(points, idx):
    """
    Input:
    ...
    Return:
        new_points: indexed points data, [B, S, K, C]
    """
    B, N, C = points.shape  # 获取 points 的形状
    B, S, K = idx.shape  # 获取 idx 的形状

    # 使用 idx 对 points 进行索引，构造 [B, S, K, C] 形状的点云数据
    # 我们不使用 expand，而是通过 gather 或其他手段直接通过 idx 获取对应点
    res = points[torch.arange(B).view(-1, 1, 1), idx]  # 索引，形状为 [B, S, K, C]
...
def partition_based_pooling(points: torch.Tensor, points_features: torch.Tensor, grid_size: list[float, float, float] = [0.02,0.02,0.02]) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    for i in range(batch_size):
        ...
        cluster_indices = GridCluster(points[i], torch.tensor(grid_size, device=points[i].device))
        ...
        pooled_features.scatter_reduce_(0, inverse_indices[:, None].expand(-1, num_features), points_features[i], reduce="amax")
        ...
        # add by xlxlqqq
        cluster_counts = torch.zeros(num_clusters, dtype=torch.float, device=inverse_indices.device)
        cluster_counts.scatter_add_(0, inverse_indices, torch.ones_like(inverse_indices, dtype=torch.float))
        cluster_counts = cluster_counts.unsqueeze(1)
        # add by xlxlqqq
...
class TransitionUpBlock(nn.Module):
    ...
    def forward(self, points_xyz, points_features, skipped_xyz, skipped_features):
        ...
        interpolate_xyz, interpolate_features = interpolate(skipped_xyz, skipped_features, points_xyz, out, 1)
...
def interpolate(xyz1: torch.Tensor, features1:torch.Tensor, xyz2:torch.Tensor, features2:torch.Tensor, k = 2) -> torch.Tensor:
...
class PointTransformerV2Block(nn.Module):
    ...
    def forward(self, points_xyz, points_features):
        ...
        # print("Gather pointxyz shape:", out.shape)
        # print("Gather index shape:", indices.shape)
